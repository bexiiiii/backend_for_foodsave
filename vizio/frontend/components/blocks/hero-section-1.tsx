...
const heroBadge = {
  label: "Introducing Support for AI Models",
  href: "#link",
}

const heroCopy = {
  title: "Умное и простое решение для цифровых вывесок",
  description:
    "Ведущее ПО для цифровых вывесок упрощает создание контента для экранов — от ТВ и мониторов до планшетов и киосков.",
}

const heroActions: { primary: HeroAction; secondary: HeroAction } = {
  primary: {
...
const heroMenuItems: MenuItem[] = [
  { label: "Продукты", href: "#link" },
  { label: "Решение", href: "#link" },
  { label: "Цены", href: "#link" },
  { label: "О нас", href: "#link" },
]

const heroClientLogos: ClientLogo[] = [
  {
    name: "Nvidia Logo",
...
const logoBaseClass = "mx-auto w-fit dark:invert"

export function HeroSection() {
  return (
    <>
      <HeroHeader items={heroMenuItems} />
      <main className="overflow-hidden">
        <HeroDecorations />
        <HeroPrimarySection />
        <HeroClients logos={heroClientLogos} />
      </main>
    </>
  )
}
...
function HeroDecorations() {
  return (
    <div
      aria-hidden
      className="pointer-events-none absolute inset-0 hidden isolate opacity-50 contain-strict lg:block"
    >
...
function HeroBadge() {
  return (
    <Link
      href={heroBadge.href}
      className="bg-muted group mx-auto flex w-fit items-center gap-4 rounded-full border p-1 pl-4 shadow-md shadow-black/5 transition-all duration-300 hover:bg-background dark:border-t-white/5 dark:hover:border-t-border dark:shadow-zinc-950"
    >
      <span className="text-foreground text-sm">{heroBadge.label}</span>
    ...
    >
      <div className="rounded-[14px] border bg-foreground/10 p-0.5">
        <Button asChild size="lg" className="rounded-xl px-5 text-base">
          <Link href={heroActions.primary.href}>
            <span className="text-nowrap">{heroActions.primary.label}</span>
          </Link>
        </Button>
      </div>
      <Button
      ...
      >
        <Link href={heroActions.secondary.href}>
          <span className="text-nowrap">{heroActions.secondary.label}</span>
        </Link>
      </Button>
    </AnimatedGroup>
  )
      ...
      <div className="relative -mr-56 mt-8 overflow-hidden px-2 sm:mr-0 sm:mt-12 md:mt-20">
        ...
        <div className="inset-shadow-2xs ring-background dark:inset-shadow-white/20 bg-background relative mx-auto max-w-6xl overflow-hidden rounded-2xl border p-4 shadow-lg shadow-zinc-950/15 ring-1">
          <img
            ...
            src={heroMedia.dark.src}
            alt={heroMedia.dark.alt}
            width={heroMedia.width}
            height={heroMedia.height}
          />
          <img
            className="z-2 border-border/25 aspect-15/8 relative rounded-2xl border dark:hidden"
            src={heroMedia.light.src}
        ...
        <div className="group-hover:blur-xs mx-auto mt-12 grid max-w-2xl grid-cols-4 gap-x-12 gap-y-8 transition-all duration-500 group-hover:opacity-50 sm:gap-x-16 sm:gap-y-14">
          {logos.map((logo) => (
            <div className="flex" key={logo.name}>
              <img
                className={cn(logoBaseClass, logo.className)}
                src={logo.src}
                alt={logo.alt}
                height={20}
                width="auto"
...
function HeroHeader({ items }: { items: MenuItem[] }) {
  ...
  return (
    <header>
      <nav data-state={isMenuOpen && "active"} className="group fixed z-20 w-full px-2">
        <div className={navWrapperClass}>
          <div className="relative flex flex-wrap items-center justify-between gap-6 py-3 lg:gap-0 lg:py-4">
            <div className="flex w-full justify-between lg:w-auto">
              ...
              >
                <Menu className="m-auto size-6 duration-200 group-data-[state=active]:scale-0 group-data-[state=active]:opacity-0 group-data-[state=active]:rotate-180" />
                <X className="absolute inset-0 m-auto size-6 -rotate-180 scale-0 opacity-0 duration-200 group-data-[state=active]:rotate-0 group-data-[state=active]:scale-100 group-data-[state=active]:opacity-100" />
              </button>
            </div>

            <DesktopNav items={items} />
            <HeaderMenu items={items} isScrolled={isScrolled} />
          </div>
        </div>
      </nav>
    </header>
...
function HeaderMenu({ items, isScrolled }: { items: MenuItem[]; isScrolled: boolean }) {
  return (
    <div className="dark:shadow-none dark:lg:bg-transparent bg-background group-data-[state=active]:block mb-6 hidden w-full flex-wrap items-center justify-end space-y-8 rounded-3xl border p-6 shadow-2xl shadow-zinc-300/20 md:flex-nowrap lg:m-0 lg:flex lg:w-fit lg:gap-6 lg:space-y-0 lg:border-transparent lg:bg-transparent lg:p-0 lg:shadow-none">
